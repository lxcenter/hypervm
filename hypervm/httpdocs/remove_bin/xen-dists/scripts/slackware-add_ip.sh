...
function fix_rcinet1()
{
        ...
        sed -e 's/eth/eth/g' -e 's/^[\ \t]*\/sbin\/route add default gw .*/\t\/sbin\/route add -net ${NETWORK_GATEWAY_NET}\/24 dev eth0; \/sbin\/route add default gw \${GATEWAY} dev eth0/' < ${file} > ${file}.$$ && mv -f ${file}.$$ ${file}
