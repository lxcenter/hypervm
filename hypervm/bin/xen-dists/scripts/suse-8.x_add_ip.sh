...
BROADCAST=0.0.0.0" > $IFCFG || error "Can't write to file $IFCFG" $VZ_FS_NO_DISK_SPACE
	...
	echo "${FAKEGATEWAY}	0.0.0.0 255.255.255.255	${ETH_DEV}	
default	${FAKEGATEWAY}	0.0.0.0	${ETH_DEV}" > ${ROUTES} || error "Can't write to file $IFCFG" $VZ_FS_NO_DISK_SPACE
	# Set up /etc/hosts
	if [ ! -f ${HOSTFILE} ]; then
...
function create_config()
{
	...
	echo "STARTMODE=onboot
IPADDR=${ip}" > ${IFCFG_DIR}/bak/${ETH_DEV_CFG}:${ifnum} || \
	error "Can't write to file ${IFCFG_DIR}/${ETH_DEV_CFG}:${ifnum}" ${VZ_FS_NO_DISK_SPACE}
}
...
function add_ip()
{
	...
	if [ "x${VE_STATE}" = "xrunning" ]; then
		...
		ifup ${ETH_DEV} -o doalias >/dev/null 2>&1
	fi
}
